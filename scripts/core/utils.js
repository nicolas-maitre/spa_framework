...
Utils.decodeTextTemplate = function (string="", { startStr = "{{", endStr = "}}" } = {}) {
	if(!string.includes(startStr) || !string.includes(endStr)){
		return {};
	}
	var textSections = string.split(startStr);
	var textArray = [];
	textSections.forEach(section => {
		section.split(endStr);
	});
};
